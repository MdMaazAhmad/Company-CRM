...
// Shared edit fields — identical shape to Manage's PlanFields.
...
export default function PlanDetailClient({ plan }: { plan: Plan }) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <Link href="/manage" className="text-sm text-muted hover:text-ink">
        ← Back to Manage
      </Link>

      <div className="flex items-start justify-between gap-4">
        <PageHeader
          eyebrow={`Plan · ${plan.category}`}
          title={plan.name}
          subtitle={plan.tagline ?? undefined}
        />
        <FormDialog
          trigger={
      ...
      <div className="rounded-2xl border border-line bg-surface p-6">
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            ...
            <div className="font-heading text-2xl text-brand tabular-nums">
              {inr(plan.breakPrice)}
            </div>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-brand/30 bg-brand-soft/30 p-6">
        ...
        <dl className="space-y-3 text-sm">
          ...
          {plan.pitch && (
            <div>
              <dt className="text-muted">Opening pitch</dt>
              <dd className="text-ink">{plan.pitch}</dd>
            </div>
          )}
          {plan.objection && (
            <div>
              <dt className="text-muted">Key objection</dt>
              <dd className="text-ink">{plan.objection}</dd>
            </div>
          )}
          {plan.upsell && (
            <div>
              <dt className="text-muted">Upsell / note</dt>
              <dd className="text-ink">{plan.upsell}</dd>
            </div>
          )}
        </dl>
      </div>
